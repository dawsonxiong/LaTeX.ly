...
def merge_equals(contours):
    ...
    while i < len(contours):
        ...
        if i + 1 < len(contours):
            ...
            if is_equals_sign(w1, h1, w2, h2):
                # Merge the two contours
                x_pad = min(x1, x2)
                y_pad = min(y1, y2)
                w_pad = max(x1 + w1, x2 + w2) - x_pad
                h_pad = max(y1 + h1, y2 + h2) - y_pad

                merged_contour = np.array([[x_pad, y_pad], [x_pad + w_pad, y_pad],
                                          [x_pad + w_pad, y_pad + h_pad], [x_pad, y_pad + h_pad]], dtype=np.int32)
...
def resize_image(image):
     ...
     width = image.shape[1]
     dimensions = ((int (width*(40/height))),40)
